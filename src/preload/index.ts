...
import { IServerConf, IServerOption, IServerSettings, ServerCore } from '@/types/Server'
...
export interface IElectronAPI {
  ...
  fs: {
    ...
    readdirWithTypes: (folderPath: string) => Promise<{ path: string; type: 'file' | 'folder' }[]>
    archiveFiles: (filesToArchive: string[], zipPath: string) => Promise<boolean>
  ...
  version: {
    ...
    init: (settings: TSettings, versionConf: IVersionConf) => Promise<IVersionClassData>
    install: (
      settings: TSettings,
  ...
  auth: {
    ...
    microsoftRefresh: (refreshToken: string, id: string) => Promise<IRefreshTokenResponse | null>
    elyby: (code: string) => Promise<IAuthResponse | null>
    elybyRefresh: (refreshToken: string, id: string) => Promise<IRefreshTokenResponse | null>
    discord: (code: string) => Promise<IAuthResponse | null>
    discordRefresh: (refreshToken: string, id: string) => Promise<IRefreshTokenResponse | null>
    startServer: () => Promise<{
      code: string
  ...
  backend: {
    ...
    shareModpack: (at: string, modpack: { conf: IModpack['conf'] }) => Promise<string>
    updateModpack: (at: string, shareCode: string, update: IModpackUpdate) => Promise<boolean>
    deleteModpack: (at: string, shareCode: string) => Promise<void>
    updateUser: (at: string, id: string, user: IUpdateUser) => Promise<IUser | null>
  ...
  game: {
    runJar: (command: string, args: string[], cwd: string) => Promise<number | string>
    installServer: (command: string, args: string[], serverPath: string) => Promise<string | number>
    runGame: (
      versionName: string,
      instance: number,
      command: string,
  ...
  mods: {
    ...
    downloadOther: (settings: TSettings, versionConf: IVersionConf) => Promise<boolean>
  }
  other: {
  ...
  server: {
    ...
    updateProperties: (filePath: string, settings: IServerSettings) => Promise<void>
  }
  skins: {
    load: (
      launcherPath: string,
    ...
    uploadSkin: (userId: string, platform: string, skinPath: string) => Promise<SkinsData | null>
    deleteSkin: (
      userId: string,
      platform: string,
      skinId: string,
  ...
  modManager: {
    ...
    getFilter: (provider: Provider, projectType: ProjectType) => Promise<IFilterGroup[]>
    getProject: (provider: Provider, projectId: string) => Promise<IProject | null>
    getVersions: (
      provider: Provider,
      projectId: string,
  ...
  worlds: {
    ...
    readWorld: (worldPath: string, account: ILocalAccount) => Promise<IWorld | null>
  ...
  events: {
    ...
    onConsoleClear: (callback: (versionName: string, instance: number) => void) => void
...
export const api = {
  ...
  fs: {
    ...
    readdirWithTypes: (folderPath: string) => ipcRenderer.invoke('fs:readdirWithTypes', folderPath),
    ...
    isDirectory: (targetPath: string) => ipcRenderer.invoke('fs:isDirectory', targetPath),
    getDirectories: (source: string) => ipcRenderer.invoke('fs:getDirectories', source)
  },
  clipboard: {
  ...
  file: {
    ...
    getTotalSizes: (filePaths: string[]) => ipcRenderer.invoke('file:getTotalSizes', filePaths),
    getFile: (filePath: string) => ipcRenderer.invoke('file:getFile', filePath),
    fromBuffer: (data: ArrayBuffer) => ipcRenderer.invoke('file:fromBuffer', data),
  ...
  version: {
    ...
    install: (
      ...
      account: ILocalAccount,
      extraItems?: DownloadItem[]
    ) => ipcRenderer.invoke('version:install', settings, versionConf, account, extraItems),
    getRunCommand: (
      settings: TSettings,
      versionConf: IVersionConf,
      account: ILocalAccount,
    ...
    run: (
      ...
      ipcRenderer.invoke('version:run', settings, versionConf, account, authData, instance, quick),
  ...
  accounts: {
    save: (accounts: IAccountConf['accounts'], lastPlayed: string, launcherPath: string) =>
  ...
  backend: {
    getModpack: (at: string, code: string) => ipcRenderer.invoke('backend:getModpack', at, code),
    ...
    getUser: (at: string, id: string) => ipcRenderer.invoke('backend:getUser', at, id),
    uploadFileFromPath: (at: string, filePath: string, fileName?: string, folder?: string) =>
  ...
  versions: {
    ...
    getLoaderVersions: (loader: 'forge' | 'neoforge' | 'fabric' | 'quilt', versionId: string) =>
  ...
  game: {
    ...
    runGame: (
      ...
      args: string[],
      cwd: string
    ) => ipcRenderer.invoke('game:runGame', versionName, instance, command, args, cwd),
    closeGame: (versionName: string, instance: number) =>
      ipcRenderer.invoke('game:closeGame', versionName, instance)
  },
  java: {
    getPath: (majorVersion: number) => ipcRenderer.invoke('java:getPath', majorVersion),
    install: (majorVersion: number) => ipcRenderer.invoke('java:install', majorVersion)
  },
  mods: {
    check: (settings: TSettings, versionConf: IVersionConf, server?: IServerConf) =>
  ...
  server: {
    ...
    getSettings: (filePath: string) => ipcRenderer.invoke('server:getSettings', filePath),
  ...
  skins: {
    load: (
      ...
      nickname: string,
      accessToken: string
    ) => ipcRenderer.invoke('skins:load', launcherPath, platform, userId, nickname, accessToken),
    ...
    changeModel: (userId: string, platform: string, model: 'classic' | 'slim') =>
    ...
    deleteSkin: (userId: string, platform: string, skinId: string, type: 'skin' | 'cape') =>
    ...
    importByUrl: (userId: string, platform: string, url: string, type: 'skin' | 'cape') =>
      ipcRenderer.invoke('skins:importByUrl', userId, platform, url, type),
    importByFile: (userId: string, platform: string, filePath: string, type: 'skin' | 'cape') =>
      ipcRenderer.invoke('skins:importByFile', userId, platform, filePath, type),
    ...
    renameSkin: (userId: string, platform: string, skinId: string, newName: string) =>
  ...
  modManager: {
    search: (query: string, provider: Provider, options: any, pagination: any) =>
      ipcRenderer.invoke('modManager:search', query, provider, options, pagination),
    getSort: (provider: any) => ipcRenderer.invoke('modManager:getSort', provider),
  ...
  rpc: {
    updateActivity: (activity: any) => ipcRenderer.invoke('rpc:updateActivity', activity)
  },
  skin: {
  ...
  events: {
    onConsoleChangeStatus: (
      ...
      ipcRenderer.on('consoleChangeStatus', (_event, versionName, instance, status) => {
        callback(versionName, instance, status)
      })
    },

    onConsoleMessage: (callback: (versionName: string, instance: number, message: any) => void) => {
      ipcRenderer.on('consoleMessage', (_event, versionName, instance, message) => {
        callback(versionName, instance, message)
      })
    },

    onConsoleClear: (callback: (versionName: string, instance: number) => void) => {
