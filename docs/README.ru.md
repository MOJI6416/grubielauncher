<p align="center">
  <img src="../resources/icon.png" alt="Grubie Launcher Logo" width="160"/>
</p>

<p align="center">
  <a href="https://grubielauncher.com/">grubielauncher.com</a>
</p>

<p align="center">
  <img src="https://img.shields.io/github/v/release/MOJI6416/grubielauncher" alt="Version">
  <img src="https://img.shields.io/github/license/MOJI6416/grubielauncher" alt="License">
  <img src="https://img.shields.io/badge/platform-Windows%20%7C%20Linux-blue" alt="Platforms">
  <br/>
  <a href="https://discord.com/invite/URrKha9hk7" target="_blank">
    <img src="https://img.shields.io/discord/1229487451916210236?label=Discord&logo=discord&color=7289DA" alt="Discord">
  </a>
</p>

---

# О проекте

**Grubie Launcher** — современный лаунчер для Minecraft с упором на удобство, социальные функции и управление сборками. Он помогает устанавливать версии, управлять модами и мирами, делиться сборками и поднимать серверы прямо из лаунчера.

<kbd>[<img title="English" alt="English" src="https://upload.wikimedia.org/wikipedia/commons/thumb/a/a5/Flag_of_the_United_Kingdom_%281-2%29.svg/330px-Flag_of_the_United_Kingdom_%281-2%29.svg.png" width="22">](https://github.com/MOJI6416/grubielauncher/blob/main/README.md)</kbd>
<kbd>[<img title="Ukraine" alt="Ukraine" src="https://upload.wikimedia.org/wikipedia/commons/thumb/4/49/Flag_of_Ukraine.svg/1280px-Flag_of_Ukraine.svg.png" width="22">](https://github.com/MOJI6416/grubielauncher/blob/main/docs/README.uk.md)</kbd>

# Полезные ссылки

- **Веб-сайт:** [grubielauncher.com](https://grubielauncher.com/)
- **Загрузки / Релизы:** [github.com/MOJI6416/grubielauncher/releases](https://github.com/MOJI6416/grubielauncher/releases)
- **Политика конфиденциальности:** [grubielauncher.com/privacy](https://grubielauncher.com/privacy)

# Возможности

- Установка и запуск **официальных версий Minecraft**.
- Поддержка **Forge**, **Fabric**, **NeoForge** и **Quilt**.
- Автоматическая загрузка Java и зависимостей.
- Поддержка нескольких аккаунтов: **Microsoft/Mojang**, **Ely.by**, **Discord** и оффлайн.
- Удобное управление версиями, модами и мирами.
- Интеграция с **CurseForge** и **Modrinth**.
- Система скинов: Microsoft, Ely.by и **собственные** скины.
- Система друзей.
- Достижения и внутренняя статистика игрока.
- Система публикации сборок: игроки вашей сборки получают обновления автоматически.
- Функция **«Открыть доступ к миру»** через интернет с режимами **public** и **friends**.
- **«Играть вместе» в один клик**: у друга открыт мир — жми «Играть», лаунчер сам установит и синхронизирует его сборку и подключит к миру.
- **Диагностика крашей**: лаунчер читает crash-report и объясняет причину и решение (правила обновляются без релиза).
- **ИИ-разбор краша**: если причина не определилась автоматически, лаунчер может отправить краш на разбор ИИ и показать вероятную причину, подозреваемые моды и что попробовать.
- **Каталог публичных сборок** на [grubielauncher.com](https://grubielauncher.com/packs) с кнопкой «Открыть в лаунчере».
- **Drag & drop импорт** .mrpack/.zip сборок из CurseForge, Modrinth, Prism и экспортов Grubie.
- Встроенный **тест доступности** ресурсов лаунчера и официальных сервисов (помогает выявить блокировки провайдера).
- Создание и управление сервером прямо из лаунчера. _Поддерживаемые ядра серверов: Vanilla, Spigot, Bukkit, Paper, Purpur, Forge, NeoForge, Fabric, Quilt._

# Установка и запуск

1. Скачайте последнюю версию из [Releases](https://github.com/MOJI6416/grubielauncher/releases).
2. Запустите `grubie-launcher`.

_(Поддерживаемые платформы: Windows, Linux)_

# Лицензия

Проект распространяется по лицензии **MIT**.
