...
![Вкладка «Контент» сборки с поиском по Modrinth](screenshots/ru/mods.webp)
...
**Стек.** Electron с electron-vite, React 19 и React Compiler, TypeScript, Tailwind CSS 4 с Radix UI, jotai для состояния, TanStack Query для серверных данных, i18next для трёх языков интерфейса, Vitest для тестов. CI собирает на Node 22.

```bash
npm install         # после установки electron-builder пересобирает нативные зависимости
npm run dev         # лаунчер с горячей перезагрузкой
npm run typecheck   # проекты TypeScript главного процесса и интерфейса
npm test            # Vitest
npm run lint        # ESLint
npm run build       # проверка типов + сборка бандла
...
| Путь | Что внутри |
| --- | --- |
| `main/` | Главный процесс Electron: установка и запуск версий, Java, аккаунты, провайдеры модов, доступ к миру, сервер и все обработчики IPC. |
| `preload/` | Единственный мост в интерфейс — поверхность `window.api`. |
| `renderer/src/` | Интерфейс на React. |
| `shared/` | Код, нужный обоим процессам: настройки, файлы сборок, очистка логов, имена версий. |
| `types/` | Типы, общие для всего приложения. |

Интерфейс собран вокруг функций. `shell/` — рама вокруг всего: боковая панель, верхняя панель, палитра команд; `navigation/` — маршруты и история «назад/вперёд»; `screens/` — по экрану на маршрут; `features/<имя>/` — компоненты функции рядом с её чистой логикой; `agent/` — цикл ассистента и его инструменты.

Тесты лежат рядом с кодом как `*.test.ts` и выполняются в обычном окружении Node, поэтому логику, которую стоит проверять, держат в чистых модулях отдельно от компонентов.
